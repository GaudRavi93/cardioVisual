...
@Component({
  selector: 'page-intro',
  templateUrl: 'orientation-playlist-hcp.html',
 
})
export class OrientationPlaylistHcp {
  ...
  constructor(public navCtrl: NavController,public injector: Injector) {
    ...
    setTimeout(() => { 
      this.network_check_object.getPageAction('OrientationPlaylistHcp','start','',localStorage.getItem('cv5usrType'));
    }, 1000); 
      ...
      this.database_provider_object.fetchAllData('custom_listing' , query).then(custom_listing_data => {
        ...
        this.database_provider_object.fetchAllData('new_sub_topics', condition_query).then(conditionData => {
          if(conditionData.length > 0){
            this.navCtrl.push('VideoDetailsPage', { data :conditionData[0], tag:tag, previousPage:'OrientationPlaylistHcp', 'header':'hide', 'request_from': 'tag','sendTo':'OrientationPlaylist' }); 
          }
        });
      });
    }
